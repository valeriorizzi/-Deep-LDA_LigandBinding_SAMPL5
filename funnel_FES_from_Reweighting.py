...
#funnel options
parser.add_argument('--rfunnel',dest='rfunnel',type=float,required=True,help='the radius of the funnel, it MUST be in nm, the units of the CV z')
parser.add_argument('--uat',dest='uat',type=float,required=True,help='the value of the z CV above which the ligand is unbound')
parser.add_argument('--bat',dest='bat',type=float,required=True,help='the value of the z CV below which the ligand is bound')
...
if args.rfunnel is not None:
  ...
  prefactorfunnel=np.log(pref) #metti pure kT, così che è la correzione che si somma
...
#funnel
  if calc_funnel:
    if not dim2:
      ...
      fes2=(fes-shift2)
      deltafunnel=-(-kbt*np.logaddexp.reduce(-1/kbt*fes2[grid_cv_x<bat])-kbt*(prefactorfunnel+extrafactor))
...
#actual printing
  ...
  f.write('#! SET weights %g\n'%effsize2)
  if calc_deltaF:
    f.write('#! SET DeltaF %g\n'%(deltaF))
  if calc_funnel:
    f.write('#! SET fundeltaF %g\n'%(deltafunnel))
  #if block_av:
  #  f.write('#! SET block_weight %g\n'%(safe_block_weight))
